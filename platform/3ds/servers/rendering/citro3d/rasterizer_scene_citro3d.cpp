...
void RasterizerSceneCitro3D::render_scene(const Ref<RenderSceneBuffers> &p_render_buffers, const CameraData *p_camera_data, const CameraData *p_prev_camera_data, const PagedArray<RenderGeometryInstance *> &p_instances, const PagedArray<RID> &p_lights, const PagedArray<RID> &p_reflection_probes, const PagedArray<RID> &p_voxel_gi_instances, const PagedArray<RID> &p_decals, const PagedArray<RID> &p_lightmaps, const PagedArray<RID> &p_fog_volumes, RID p_environment, RID p_camera_attributes, RID p_shadow_atlas, RID p_occluder_debug_tex, RID p_reflection_atlas, RID p_reflection_probe, int p_reflection_probe_pass, float p_screen_mesh_lod_threshold, const RenderShadowData *p_render_shadows, int p_render_shadow_count, const RenderSDFGIData *p_render_sdfgi_regions, int p_render_sdfgi_region_count, const RenderSDFGIUpdateData *p_sdfgi_update_data, RenderingMethod::RenderInfo *r_info) {
	RendererCitro3D::TextureStorage *texture_storage = RendererCitro3D::TextureStorage::get_singleton();
	Ref<RenderSceneBuffersCitro3D> rb;
	if (p_render_buffers.is_valid()) {
		rb = p_render_buffers;
		ERR_FAIL_COND(rb.is_null());
	}
	RendererCitro3D::RenderTarget *rt = texture_storage->get_render_target(rb->render_target);
	ERR_FAIL_COND(!rt);
...
Ref<RenderSceneBuffers> RasterizerSceneCitro3D::render_buffers_create() {
	Ref<RenderSceneBuffersCitro3D> rb;
	rb.instantiate();
	return rb;
}