...
def dummy_validator(_key, _val, _env):
    return True
...
def verify_devkitpro_install(dkp_path):
    ...
    if (not os.path.isfile(dkp_path + "/" + "devkitARM/3ds_rules")):
        ...
        return False
    if (not os.path.isfile(dkp_path + "/libctru/lib/libcitro3d.a") or not os.path.isfile(
            dkp_path + "/libctru/lib/libctru.a")):
    ...
    if (not os.path.isfile(dkp_path + "/portlibs/3ds/lib/libz.a")):
        ...
        return False
    if (not os.path.isfile(dkp_path + "/tools/bin/3dsxtool.exe") and not os.path.isfile(
            dkp_path + "/tools/bin/3dsxtool")):
        print("Although the devkitPro path exists, you do not have 3dsxtool installed.")
        return False
    if (not os.path.isfile(dkp_path + "/tools/bin/smdhtool.exe") and not os.path.isfile(
            dkp_path + "/tools/bin/smdhtool")):
        print("Although the devkitPro path exists, you do not have smdhtool installed.")
        return False
    if (not os.path.isfile(dkp_path + "/tools/bin/picasso.exe") and not os.path.isfile(
            dkp_path + "/tools/bin/picasso")):
...
def configure_cross(env):
    ...
    env.Append(CCFLAGS=['-D__3DS__', '-ffast-math', '-DLIBC_FILEIO_ENABLED', '-DNO_SAFE_CAST',
                        '-D_XOPEN_SOURCE=500', '-DRW_LOCK_H', '-DUNIX_SOCKET_UNAVAILABLE'])
    env.Append(CFLAGS=['-Wincompatible-pointer-types', '-Wno-implicit-function-declaration', '-Wno-parentheses'])
    env.Append(CPPPATH=[dkp_path + "/portlibs/3ds/include", dkp_path + "/portlibs/3ds/include/freetype2", dkp_path +
                        "/portlibs/armv6k/include", dkp_path + "/libctru/include",
                        dkp_path + "/devkitARM/arm-none-eabi/include"])
    ...
    env.Append(
        CCFLAGS=['-U__INT32_TYPE__', '-U__UINT32_TYPE__', '-D__INT32_TYPE__=int', '-D__UINT32_TYPE__=unsigned int',
                 '-U__UINT32_MAX__', '-U__INT32_MAX__', '-D__UINT32_MAX__=4294967295U', '-D__INT32_MAX__=2147483647'])
